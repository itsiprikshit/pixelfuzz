...
process = Process(target=openCalculator)
process.start()
...
def getWindowCoords():
    #windowdid_command = "xwininfo |sed -e 's/^ *//' | grep -E \"Window id\" | awk '{ print $4 }'"
    #windowdid_command = "xprop -root _NET_ACTIVE_WINDOW"
    #windowid = subprocess.check_output(windowdid_command,shell=True)
    #windowid = windowid.decode('utf-8')
...
# print("New number : "+str(random_number))
